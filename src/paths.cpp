...
AppPaths default_app_paths() {
  ...
  return {.config_file = app_data / "Horcrux" / "config.json",
          .state_directory = local_app_data / "Horcrux"};
#elif defined(__APPLE__)
  const auto home = environment_path("HOME", ".");
  const auto base = home / "Library" / "Application Support" / "Horcrux";
  ...
  return {.config_file = config_home / "horcrux" / "config.json",
          .state_directory = state_home / "horcrux"};
#endif
}
